...
namespace UZonMailService.Models.SqlLite.EmailSending
{
    ...
    public class SendingItemExcelData:JObject
    {
        public SendingItemExcelData(JObject row):base(row)
        {
            ...
            CC = [.. row.SelectTokenOrDefault("cc", string.Empty).SplitBySeparators()];
            BCC = [.. row.SelectTokenOrDefault("bcc", string.Empty).SplitBySeparators()];
            ...
            Proxy = row.SelectTokenOrDefault("proxy", string.Empty);
