...
namespace Uamazing.SME.Server.Controllers
{
    ...
    /// 分组的控制器
    /// </summary>
    public class InboxGroupController : CurdController<InboxGroup>
    {
        ...
        public InboxGroupController(CurdService<InboxGroup> curdService,  IOptions<TokenParams> options) : base(curdService)
        {
        ...
        ///// <returns></returns>
        public override async Task<ResponseResult<InboxGroup>> Create([FromBody] InboxGroup data)
        {
            ...
            // 判断组名是否重复
            if (CurdService.GetFirstOrDefault(x => x.Name == data.Name) != null) return new ErrorResponse<InboxGroup>($"{data.Name} 已经存在");
