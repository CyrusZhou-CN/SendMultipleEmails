...
namespace Uamazing.UZonEmail.Server.Services
{
    ...
    /// 文件服务
    /// </summary>
    public class FileObjectService : CRUDService
    {
        ...
        public async Task<FileObject> GetFileObject(string sha256)
        {
            ...
            var fileObj = await FirstOrDefault<FileObject>(x=>x.SHA256 == sha256);
            return fileObj;
        }
        ...
        {
            var diskFile = await FirstOrDefault<PhysicalFileObject>(x => x.Id == fileId) ?? throw new NullReferenceException($"文件:{fileName} 不存在");
